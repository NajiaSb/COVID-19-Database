...
SET echo OFF
PROMPT Press enter to continue to QUERY 7...
ACCEPT cont;
SET echo ON
--------------------------------------------------------------------------------
-- QUERY 7: Get the smallest value for each month of reported column by user 
-- input from CasesByProvince table.  
SET echo OFF
PROMPT Enter one of the following columns: confirmedPositive, resolved, deaths, totalCases, hospitalizedPatients, icuPatients to find the minimum value for each month 
ACCEPT ans CHAR PROMPT "ans: "
SET echo ON
SELECT 
    EXTRACT(YEAR FROM C.reportedDate) AS "year",
    EXTRACT(MONTH FROM C.reportedDate) AS "month", 
    MIN(C.&ans) AS "min(&ans)" 
FROM CasesByProvince  C 
GROUP BY 
    EXTRACT(YEAR FROM C.reportedDate),
    EXTRACT(MONTH FROM C.reportedDate) 
